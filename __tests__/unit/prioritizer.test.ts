...
describe('assignPriority', () => {
  ...
  describe('URGENT', () => {
    it('POSSIBLE_DUPLICATE siempre es URGENT con confidence alta', () => {
      expect(assignPriority({ amount: 100 }, 'POSSIBLE_DUPLICATE', 0.99, materiality)).toBe('URGENT');
    });

    it('POSSIBLE_DUPLICATE siempre es URGENT con confidence baja', () => {
      expect(assignPriority({ amount: 100 }, 'POSSIBLE_DUPLICATE', 0.50, materiality)).toBe('URGENT');
    });
  ...
  describe('DECISION', () => {
    it('UNIDENTIFIED con importe por encima de materialidad', () => {
      expect(assignPriority({ amount: 1000 }, 'UNIDENTIFIED', 0.80, materiality)).toBe('DECISION');
    });

    it('confidence por debajo de 0.70 con tipo normal', () => {
      expect(assignPriority({ amount: 100 }, 'MATCH_SIMPLE', 0.60, materiality)).toBe('DECISION');
    });

    it('confidence 0 con tipo no urgente', () => {
      expect(assignPriority({ amount: 100 }, 'EXPENSE_NO_INVOICE', 0, materiality)).toBe('DECISION');
    });
  });
  ...
  describe('CONFIRMATION', () => {
    it('confidence entre 0.70 y 0.89 con tipo normal', () => {
      expect(assignPriority({ amount: 100 }, 'MATCH_SIMPLE', 0.80, materiality)).toBe('CONFIRMATION');
    });

    it('MATCH_PARTIAL con confidence alta sigue siendo CONFIRMATION', () => {
      expect(assignPriority({ amount: 100 }, 'MATCH_PARTIAL', 0.95, materiality)).toBe('CONFIRMATION');
    });

    it('MATCH_DIFFERENCE con confidence alta sigue siendo CONFIRMATION', () => {
      expect(assignPriority({ amount: 100 }, 'MATCH_DIFFERENCE', 0.95, materiality)).toBe('CONFIRMATION');
    });
  });
  ...
  describe('ROUTINE', () => {
    it('confidence >= 0.90 con MATCH_SIMPLE', () => {
      expect(assignPriority({ amount: 100 }, 'MATCH_SIMPLE', 0.90, materiality)).toBe('ROUTINE');
    });

    it('confidence >= 0.90 con EXPENSE_NO_INVOICE', () => {
      expect(assignPriority({ amount: -50 }, 'EXPENSE_NO_INVOICE', 0.95, materiality)).toBe('ROUTINE');
    });
  });
  ...
  describe('Edge cases', () => {
    it('confidence exactamente 0.70 es CONFIRMATION (< es estricto)', () => {
      expect(assignPriority({ amount: 100 }, 'MATCH_SIMPLE', 0.70, materiality)).toBe('CONFIRMATION');
    });
