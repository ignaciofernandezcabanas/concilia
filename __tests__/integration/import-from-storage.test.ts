...
describe("Import from Storage", () => {
  ...
  it("escanea carpeta e importa PDFs", async () => {
    mockStorageProvider.listFiles.mockResolvedValue([
      { id: "f1", name: "fra-001.pdf", mimeType: "application/pdf", size: 1024, createdAt: "", modifiedAt: "" },
      { id: "f2", name: "fra-002.pdf", mimeType: "application/pdf", size: 2048, createdAt: "", modifiedAt: "" },
      { id: "f3", name: "readme.txt", mimeType: "text/plain", size: 50, createdAt: "", modifiedAt: "" },
    ]);
    mockStorageProvider.downloadFile.mockResolvedValue(Buffer.from("pdf"));
    mockExtractInvoice.mockResolvedValue({
      number: "FRA", totalAmount: 1000, confidence: 0.85, type: "RECEIVED", currency: "EUR", lines: [],
    });
  ...
  it("ignora archivos no-PDF", async () => {
    mockStorageProvider.listFiles.mockResolvedValue([
      { id: "f1", name: "data.xlsx", mimeType: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", size: 500, createdAt: "", modifiedAt: "" },
    ]);
  ...
  it("dedup: PDF ya importado → skipped", async () => {
    mockStorageProvider.listFiles.mockResolvedValue([
      { id: "f1", name: "fra.pdf", mimeType: "application/pdf", size: 1024, createdAt: "", modifiedAt: "" },
    ]);
  ...
  it("OCR confidence < 0.50 → error", async () => {
    mockStorageProvider.listFiles.mockResolvedValue([
      { id: "f1", name: "borroso.pdf", mimeType: "application/pdf", size: 1024, createdAt: "", modifiedAt: "" },
    ]);
    mockStorageProvider.downloadFile.mockResolvedValue(Buffer.from("pdf"));
    mockExtractInvoice.mockResolvedValue({ totalAmount: null, confidence: 0.30, lines: [] });
  ...
  it("maxFiles: 2 con 5 PDFs → hasMore=true", async () => {
    mockStorageProvider.listFiles.mockResolvedValue(
      Array.from({ length: 5 }, (_, i) => ({
        id: `f${i}`, name: `f${i}.pdf`, mimeType: "application/pdf", size: 100, createdAt: "", modifiedAt: "",
      }))
    );
    mockStorageProvider.downloadFile.mockResolvedValue(Buffer.from("pdf"));
    mockExtractInvoice.mockResolvedValue({ number: "X", totalAmount: 100, confidence: 0.80, type: "RECEIVED", currency: "EUR", lines: [] });
