...
import { buildBankTransaction, buildInvoice, buildReconciliation, buildContact } from '../helpers/factories';
...
describe('resolveItem — Unified Resolver', () => {
  ...
  describe('approve', () => {
    it('actualiza reconciliation a APPROVED y tx a RECONCILED', async () => {
      ...
      mockPrisma.reconciliation.findFirstOrThrow.mockResolvedValue({ ...reco, bankTransaction: bankTx, invoice: null });
      mockPrisma.reconciliation.update.mockResolvedValue({});
      mockPrisma.bankTransaction.update.mockResolvedValue({});

      const result = await resolveItem({ action: 'approve', reconciliationId: 'reco_1' }, 'user_1', 'company_1');
    ...
    it('llama a updateInvoicePaymentStatus si hay invoice', async () => {
      ...
      mockPrisma.reconciliation.findFirstOrThrow.mockResolvedValue({ ...reco, bankTransaction: bankTx, invoice });
      mockPrisma.reconciliation.update.mockResolvedValue({});
      mockPrisma.bankTransaction.update.mockResolvedValue({});
    ...
    it('sin invoice → no rompe', async () => {
      ...
      mockPrisma.reconciliation.findFirstOrThrow.mockResolvedValue({
        ...buildReconciliation(), bankTransaction: bankTx, invoice: null,
      });
      mockPrisma.reconciliation.update.mockResolvedValue({});
      mockPrisma.bankTransaction.update.mockResolvedValue({});
    ...
    it('crea audit log', async () => {
      ...
      expect(mockPrisma.auditLog.create).toHaveBeenCalledWith(
        expect.objectContaining({ data: expect.objectContaining({ action: 'reconciliation.approve' }) })
      );
    });
  });
  ...
  describe('reject', () => {
    it('reconciliation a REJECTED, tx vuelve a PENDING', async () => {
      ...
      const bankTx = buildBankTransaction();
      mockPrisma.reconciliation.findFirstOrThrow.mockResolvedValue({ ...reco, bankTransaction: bankTx });
      mockPrisma.reconciliation.update.mockResolvedValue({});
      mockPrisma.bankTransaction.update.mockResolvedValue({});

      const result = await resolveItem({ action: 'reject', reconciliationId: 'reco_1', reason: 'Incorrecto' }, 'user_1', 'company_1');
      expect(result.success).toBe(true);
      expect(mockPrisma.bankTransaction.update).toHaveBeenCalledWith(
    ...
    it('desactiva regla si matchReason empieza por "rule:"', async () => {
      ...
      await resolveItem({ action: 'reject', reconciliationId: 'reco_1', reason: 'Bad rule' }, 'user_1', 'company_1');
  ...
  describe('investigate', () => {
    it('tx a INVESTIGATING, notifica admins', async () => {
      const bankTx = buildBankTransaction();
      mockPrisma.reconciliation.findFirstOrThrow.mockResolvedValue({
        ...buildReconciliation(), bankTransaction: bankTx,
      });
      mockPrisma.reconciliation.update.mockResolvedValue({});
      mockPrisma.bankTransaction.update.mockResolvedValue({});
      mockPrisma.user.findMany.mockResolvedValue([{ id: 'admin_1' }]);

      await resolveItem({ action: 'investigate', reconciliationId: 'reco_1', note: 'Revisar' }, 'user_1', 'company_1');
      expect(mockPrisma.bankTransaction.update).toHaveBeenCalledWith(
  ...
  describe('manual_match', () => {
    it('crea reconciliation MANUAL con confidence 1.0 y actualiza invoice', async () => {
      ...
      const result = await resolveItem({
        action: 'manual_match', bankTransactionId: 'tx_1', invoiceId: 'invoice_1',
      }, 'user_1', 'company_1');

      expect(result.success).toBe(true);
      expect(mockPrisma.reconciliation.create).toHaveBeenCalledWith(
        expect.objectContaining({ data: expect.objectContaining({ type: 'MANUAL', confidenceScore: 1.0 }) })
      );
  ...
  describe('classify', () => {
    it('crea classification y tx a CLASSIFIED', async () => {
      const bankTx = buildBankTransaction();
      mockPrisma.bankTransaction.findFirstOrThrow.mockResolvedValue(bankTx);
      mockPrisma.account.findFirstOrThrow.mockResolvedValue({ id: 'acc_1', code: '629', name: 'Otros servicios', cashflowType: 'OPERATING' });
      mockPrisma.bankTransactionClassification.create.mockResolvedValue({ id: 'class_1' });
      mockPrisma.bankTransaction.update.mockResolvedValue({});

      const result = await resolveItem({
        action: 'classify', bankTransactionId: 'tx_1', accountCode: '629', cashflowType: 'OPERATING',
      }, 'user_1', 'company_1');
    ...
    it('si createRule=true → crea MatchingRule', async () => {
      const bankTx = buildBankTransaction();
      mockPrisma.bankTransaction.findFirstOrThrow.mockResolvedValue(bankTx);
      mockPrisma.account.findFirstOrThrow.mockResolvedValue({ id: 'acc_1', code: '629', name: 'Test' });
      mockPrisma.bankTransactionClassification.create.mockResolvedValue({ id: 'class_1' });
      mockPrisma.bankTransaction.update.mockResolvedValue({});

      await resolveItem({
        action: 'classify', bankTransactionId: 'tx_1', accountCode: '629', createRule: true,
      }, 'user_1', 'company_1');
  ...
  describe('mark_internal', () => {
    it('tx a INTERNAL, detectedType INTERNAL_TRANSFER', async () => {
      ...
      const result = await resolveItem({ action: 'mark_internal', bankTransactionId: 'tx_1' }, 'user_1', 'company_1');
      expect(result.success).toBe(true);
      expect(mockPrisma.bankTransaction.update).toHaveBeenCalledWith(
        expect.objectContaining({ data: expect.objectContaining({ status: 'INTERNAL', detectedType: 'INTERNAL_TRANSFER' }) })
      );
    });
  });
  ...
  describe('mark_duplicate', () => {
    it('tx a DUPLICATE, duplicateGroup actualizado si existe', async () => {
      ...
      await resolveItem({ action: 'mark_duplicate', bankTransactionId: 'tx_1' }, 'user_1', 'company_1');
      expect(mockPrisma.duplicateGroup.update).toHaveBeenCalledWith(
        expect.objectContaining({ where: { id: 'group_1' }, data: expect.objectContaining({ status: 'DUPLICATE' }) })
      );
    });
  });
  ...
  describe('mark_legitimate', () => {
    it('DuplicateGroup a LEGITIMATE, txs vuelven a PENDING', async () => {
      mockPrisma.duplicateGroup.findUniqueOrThrow.mockResolvedValue({
        id: 'group_1',
        transactions: [{ id: 'tx_1', companyId: 'company_1' }, { id: 'tx_2', companyId: 'company_1' }],
      });
      mockPrisma.duplicateGroup.update.mockResolvedValue({});
      mockPrisma.bankTransaction.updateMany.mockResolvedValue({});

      const result = await resolveItem({ action: 'mark_legitimate', duplicateGroupId: 'group_1' }, 'user_1', 'company_1');
      expect(result.success).toBe(true);
  ...
  describe('ignore', () => {
    it('tx a IGNORED, guarda note', async () => {
      ...
      await resolveItem({ action: 'ignore', bankTransactionId: 'tx_1', reason: 'No relevante' }, 'user_1', 'company_1');
      expect(mockPrisma.bankTransaction.update).toHaveBeenCalledWith(
        expect.objectContaining({ data: expect.objectContaining({ status: 'IGNORED', note: 'No relevante' }) })
      );
    });
  });
  ...
  describe('split_financial', () => {
    it('reconciliation APPROVED con resolution principal/interest', async () => {
      ...
      const result = await resolveItem({
        action: 'split_financial', reconciliationId: 'reco_1', principalAmount: 850, interestAmount: 150,
      }, 'user_1', 'company_1');
