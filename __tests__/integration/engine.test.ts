...
vi.mock("@/lib/reconciliation/detectors/internal-detector", () => ({
  detectInternalTransfer: mockDetectInternal,
}));
vi.mock("@/lib/reconciliation/detectors/intercompany-detector", () => ({
  detectIntercompany: mockDetectIntercompany,
}));
vi.mock("@/lib/reconciliation/detectors/duplicate-detector", () => ({
  detectDuplicates: mockDetectDuplicates,
}));
vi.mock("@/lib/reconciliation/detectors/return-detector", () => ({
  detectReturn: mockDetectReturn,
}));
...
vi.mock("@/lib/reconciliation/classifiers/rule-classifier", () => ({
  classifyByRules: mockClassifyRules,
}));
vi.mock("@/lib/reconciliation/classifiers/llm-classifier", () => ({
  classifyByLlm: mockClassifyLlm,
}));
...
function setupDefaults(txList = [buildBankTransaction()]) {
  ...
  mockDetectReturn.mockResolvedValue({
    isReturn: false,
    originalTxId: null,
    originalReconciliationId: null,
  });
  mockDetectFinancial.mockResolvedValue({
    isFinancial: false,
...
describe("runReconciliation — Engine", () => {
  ...
  it("duplicado detectado → bandeja, priority URGENT", async () => {
    ...
    expect(mockPrisma.bankTransaction.update).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ status: "PENDING", priority: "URGENT" }),
      })
    );
  });
  ...
  it("devolución detectada → bandeja, priority URGENT, nunca auto-aprobada", async () => {
    ...
    mockDetectReturn.mockResolvedValue({
      isReturn: true,
      originalTxId: "tx_orig",
      originalReconciliationId: "reco_orig",
    });
    ...
    mockFindFuzzy.mockResolvedValue([
      {
        ...
        suggestedDifferenceReason: "BANK_COMMISSION",
      },
    ]);
        ...
        return [
          {
            ...
            amountDifference: 20,
            differencePercent: 2,
            suggestedDifferenceReason: null,
          },
        ];
      return [];
    });
