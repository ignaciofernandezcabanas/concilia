...
describe('findFuzzyMatch', () => {
  ...
  it('match por importe dentro del 5% de tolerancia', async () => {
    ...
    const invoice = buildInvoice({ totalAmount: 1000, type: 'RECEIVED', description: 'Consultoría', contact });
  ...
  it('sin concepto → fallback a amount-only con confidence <= 0.75', async () => {
    const invoice = buildInvoice({ totalAmount: 1000, type: 'RECEIVED', description: 'Test', contact: buildContact() });
    const tx = buildBankTransaction({ amount: -990, concept: null, conceptParsed: null, counterpartName: null });
  ...
  it('confidence nunca supera 0.85', async () => {
    const contact = buildContact();
    const invoice = buildInvoice({ totalAmount: 1000, type: 'RECEIVED', description: 'Servicio exacto', contact });
  ...
  it('calcula amountDifference y differencePercent', async () => {
    const contact = buildContact();
    const invoice = buildInvoice({ totalAmount: 1000, type: 'RECEIVED', description: 'Servicio', contact });
