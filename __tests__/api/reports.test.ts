...
vi.mock("@/lib/reports/pyg-generator", () => ({
  generatePyG: vi.fn().mockResolvedValue({
    from: "2026-01-01", to: "2026-03-31", level: "titles", currency: "EUR",
    lines: [{ code: "1", label: "Ventas", amount: 10000 }],
    results: { resultadoExplotacion: 5000, resultadoFinanciero: 0, resultadoAntesImpuestos: 5000, resultadoEjercicio: 3750, ebitda: 6000 },
...
vi.mock("@/lib/reports/balance-generator", () => ({
  generateBalance: vi.fn().mockResolvedValue({
    asOf: "2026-03-31", currency: "EUR", lines: [],
    totals: { activoNoCorriente: 0, activoCorriente: 50000, totalActivo: 50000, patrimonioNeto: 30000, pasivoNoCorriente: 0, pasivoCorriente: 20000, totalPasivo: 50000 },
    generatedAt: new Date().toISOString(),
  }),
}));
...
vi.mock("@/lib/reports/forecast-generator", () => ({
  generateForecast: vi.fn().mockResolvedValue({
    currentBalance: 50000, balanceDate: "2026-03-20",
    weeks: [{ weekStart: "2026-03-23", weekEnd: "2026-03-29", expectedInflows: 5000, expectedOutflows: 3000, netFlow: 2000, projectedBalance: 52000, details: [] }],
    totals: { totalExpectedInflows: 5000, totalExpectedOutflows: 3000, projectedEndBalance: 52000 },
    horizon: 12, generatedAt: new Date().toISOString(),
  }),
}));
...
describe("Report Endpoints", () => {
  ...
  it("GET /api/reports/dashboard devuelve KPIs", async () => {
    ...
    const req = new NextRequest("http://localhost/api/reports/dashboard?from=2026-03-01&to=2026-03-31");
    const res = await GET(req);
