  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {!showCreate ? (
        ...
        <NLRuleCreator onCreated={() => { setShowCreate(false); refetch(); }} onCancel={() => setShowCreate(false)} />
      )}
      ...
      {rules.length === 0 ? (
        <div className="bg-white rounded-lg border border-subtle p-8 text-center">
          ...
          <p className="text-[11px] text-text-tertiary mt-1">Crea tu primera regla con lenguaje natural o resuelve conciliaciones marcando "recordar".</p>
        </div>
      ) : (
        <div className="bg-white rounded-lg border border-subtle overflow-hidden">
          ...
          {rules.map((rule) => (
            <div key={rule.id} className={`flex items-center h-10 px-4 text-[12px] border-b border-border-light ${rule.status !== "ACTIVE" ? "opacity-50" : ""}`}>
              <span className="w-[200px] text-text-primary truncate font-medium">
                {rule.name || rule.pattern || rule.counterpartIban || rule.conceptContains || rule.type}
              </span>
              <span className="w-24 text-text-secondary">{rule.type.replace(/_/g, " ").toLowerCase()}</span>
              <span className="w-20">
                <span className={`text-[10px] px-1.5 py-0.5 rounded ${
                  rule.origin === "MANUAL" ? "bg-accent-light text-accent" :
                  rule.origin === "INLINE" ? "bg-amber-light text-amber-text" :
                  "bg-purple-light text-purple"
                }`}>{rule.origin}</span>
              </span>
              <span className="flex-1 text-text-secondary truncate">
                {rule.action}{rule.accountCode ? ` → ${rule.accountCode}` : ""}
              </span>
              <span className="w-14 text-right font-mono text-text-primary">{rule.timesApplied}</span>
              <span className="w-16 text-center">
                <span className={`text-[10px] px-1.5 py-0.5 rounded ${
                  rule.status === "ACTIVE" ? "bg-green-light text-green-text" :
                  rule.status === "PAUSED" ? "bg-amber-light text-amber-text" :
                  "bg-hover text-text-tertiary"
                }`}>{rule.status}</span>
              </span>
              <span className="w-20 flex justify-end gap-1">
                <button onClick={() => toggleStatus(rule)} className="p-1 text-text-tertiary hover:text-text-primary" title={rule.status === "ACTIVE" ? "Pausar" : "Activar"}>
                  {rule.status === "ACTIVE" ? <Pause size={12} /> : <Play size={12} />}
                </button>
                <button onClick={() => archiveRule(rule.id)} className="p-1 text-text-tertiary hover:text-red" title="Archivar">
                  <Trash2 size={12} />
                </button>
              </span>
            </div>
  ...
  return (
    <div className="bg-white rounded-lg border border-subtle p-5">
      <div className="flex gap-2 mb-3">
        ...
        <button onClick={onCancel} className="h-10 px-3 text-[13px] text-text-secondary border border-subtle rounded-lg hover:bg-hover">
          Cancelar
        </button>
      </div>
        ...
        <div className="border border-accent rounded-lg p-4 bg-accent-light/10">
          ...
          {reasoning && (
            <details className="mb-3">
              <summary className="text-[11px] text-accent cursor-pointer font-medium">Ver razonamiento del sistema</summary>
              <div className="mt-2 p-3 bg-page rounded text-[11px] text-text-secondary space-y-1">
                {Object.entries(reasoning).map(([key, val]) => (
                  <p key={key}><span className="font-medium text-text-primary">{key}:</span> {val}</p>
                ))}
              </div>
            </details>
          )}
          ...
          <div className="flex gap-2 mt-3">
            <button onClick={handleConfirm} disabled={confirming} className="h-9 px-4 bg-green text-white text-[13px] font-medium rounded-md disabled:opacity-50">
              {confirming ? "Creando..." : "Confirmar y activar"}
            </button>
            <button onClick={() => setProposal(null)} className="h-9 px-4 text-[13px] text-text-secondary border border-subtle rounded-md hover:bg-hover">
              Descartar
            </button>
          </div>
        </div>
...
function PatternsTab() {
  ...
  return (
    <div className="flex flex-col gap-4">
        ...
        {[
          ...
          <button
            ...
            className={`px-3 py-1.5 text-[12px] font-medium rounded-md border ${
              statusFilter === f.value ? "bg-accent text-white border-accent" : "bg-white text-text-secondary border-subtle hover:bg-hover"
            }`}
          >
            {f.label}
          </button>
      ...
      {patterns.length === 0 ? (
        <div className="bg-white rounded-lg border border-subtle p-8 text-center">
          <p className="text-[13px] text-text-primary font-medium">Sin patrones {statusFilter !== "all" ? statusFilter.toLowerCase() : ""}</p>
          <p className="text-[11px] text-text-tertiary mt-1">Los patrones se detectan conforme resuelves conciliaciones.</p>
        </div>
      ) : (
        <div className="bg-white rounded-lg border border-subtle overflow-hidden">
          <div className="flex items-center h-9 px-4 border-b border-subtle text-[11px] font-semibold text-text-secondary">
          ...
          {patterns.map((p) => (
            <div key={p.id} className="flex items-center h-10 px-4 text-[12px] border-b border-border-light">
              <span className="w-28 text-text-secondary">{p.type}</span>
              <span className="flex-1 text-text-primary truncate">{p.counterpartName || p.counterpartIban || p.conceptPattern || "—"}</span>
              <span className="w-32 text-text-secondary truncate">{p.predictedReason || p.predictedAction}</span>
              <span className="w-14 text-right font-mono">{p.occurrences}</span>
              <span className={`w-20 text-right font-mono ${p.confidence >= 0.8 ? "text-green-text" : p.confidence >= 0.5 ? "text-amber-text" : "text-red-text"}`}>
                {(p.confidence * 100).toFixed(0)}%
              </span>
              <span className="w-16 text-center">
                <span className={`text-[10px] px-1.5 py-0.5 rounded ${
                  p.status === "SUGGESTED" ? "bg-amber-light text-amber-text" :
                  p.status === "ACTIVE_SUPERVISED" ? "bg-green-light text-green-text" :
                  p.status === "PROMOTED" ? "bg-purple-light text-purple" :
                  "bg-hover text-text-tertiary"
              ...
              <span className="w-28 flex justify-end gap-1">
                {p.status === "SUGGESTED" && (
                  <>
                    <button onClick={() => reviewPattern(p.id, "approve")} className="text-[10px] text-green-text hover:underline">Aprobar</button>
                    <button onClick={() => reviewPattern(p.id, "reject")} className="text-[10px] text-red-text hover:underline">Rechazar</button>
                  </>
                )}
                {p.status === "ACTIVE_SUPERVISED" && (
                  <>
                    <button onClick={() => reviewPattern(p.id, "promote")} className="text-[10px] text-accent hover:underline">Promover</button>
                    <button onClick={() => reviewPattern(p.id, "reject")} className="text-[10px] text-red-text hover:underline">Rechazar</button>
                  </>
                )}
                {p.status === "REJECTED" && (
                  <button onClick={() => reviewPattern(p.id, "approve")} className="text-[10px] text-accent hover:underline">Reactivar</button>
                )}
              </span>
            </div>
          ))}
