        ...
        {summary && (
          <div className="grid grid-cols-4 gap-4">
            ...
            <SummaryCard
              ...
              value={formatAmount(summary.totalNetBookValue)}
              accent
            />
          </div>
        )}
            ...
            {assets.map((asset) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="w-20 text-center text-text-secondary">
                    {Math.round(asset.usefulLifeMonths / 12)}a
                  </span>
...
function SummaryCard({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (
    <div className="bg-white rounded-lg border border-subtle p-4">
      ...
      <p
        className={`text-[18px] font-semibold font-mono ${accent ? "text-accent" : "text-text-primary"}`}
      >
        {value}
      </p>
    </div>
...
function CreateAssetModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-xl border border-subtle shadow-lg w-full max-w-lg p-6"
      ...
      >
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field
              ...
              onChange={(v) => update("residualValue", v)}
              type="number"
            />
            <Field
              label="Vida útil (meses)"
              value={form.usefulLifeMonths}
              onChange={(v) => update("usefulLifeMonths", v)}
              type="number"
            />
          </div>
          <div className="grid grid-cols-3 gap-3">
            <Field
              ...
              onChange={(v) => update("assetAccountCode", v)}
              placeholder="213"
            />
            <Field
              label="Cuenta amort."
              value={form.depreciationAccountCode}
              onChange={(v) => update("depreciationAccountCode", v)}
              placeholder="681"
            />
            <Field
              label="Cuenta acum."
              value={form.accumDepAccountCode}
              onChange={(v) => update("accumDepAccountCode", v)}
              placeholder="281"
            />
          </div>
        </div>
        ...
        <div className="flex justify-end gap-2 mt-4">
          ...
          <button
            ...
            disabled={!form.name || !form.acquisitionCost || saving}
