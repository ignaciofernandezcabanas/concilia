        ...
        {loading ? (
          ...
          <>
            ...
            >
              ...
              <div>
                ...
                <span className="text-[11px] text-text-tertiary ml-2">
                  al {new Date(data.balanceDate).toLocaleDateString("es-ES")}
                </span>
              </div>
              <div className="ml-auto text-right">
                <span className="text-[11px] text-text-tertiary block">
                  Proyección {data.horizon} semanas
                </span>
                <span
                  className={`text-[18px] font-semibold font-mono ${data.totals.projectedEndBalance >= 0 ? "text-green-text" : "text-red-text"}`}
                >
            ...
            <div className="bg-white rounded-lg border border-subtle p-5">
              <span className="text-[12px] font-semibold text-text-secondary mb-3 block">
                Evolución del saldo
              </span>
                ...
                return (
                  <div key={i}>
                    ...
                    >
                      ...
                      <span className="w-28 text-right font-mono text-green-text">
                        {week.expectedInflows > 0 ? `+${formatAmount(week.expectedInflows)}` : "—"}
                      </span>
                      <span className="w-28 text-right font-mono text-red-text">
                        {week.expectedOutflows > 0
                          ? `-${formatAmount(week.expectedOutflows)}`
                          : "—"}
                      </span>
                      <span
                        className={`w-28 text-right font-mono font-medium ${week.netFlow >= 0 ? "text-green-text" : "text-red-text"}`}
                      >
                        {formatAmount(week.netFlow)}
                      </span>
                      <span
                        className={`flex-1 text-right font-mono font-semibold ${week.projectedBalance >= 0 ? "text-text-primary" : "text-red-text"}`}
                      >
                      ...
                      <div className="bg-page border-b border-subtle">
                        {week.details.map((item, j) => (
                          <div key={j} className="flex items-center h-8 px-5 pl-12 text-[11px]">
                            <span
                              className={`w-14 font-medium ${item.amount > 0 ? "text-green-text" : "text-red-text"}`}
                            >
                              {item.type === "invoice_cobro"
                                ? "Cobro"
                                : item.type === "invoice_pago"
                                  ? "Pago"
                                  : "Recurrente"}
                            </span>
                            ...
                            <span className="w-16 text-right text-text-tertiary">
