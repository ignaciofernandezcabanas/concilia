...
export default function IntercompaniaPage() {
  ...
  const { data, loading, refetch } = useFetch<{ data: IntercompanyLink[]; pagination: { total: number } }>(
  ...
  async function handleConfirm(id: string) {
    await api.post(`/api/transactions/${id}/action`, { action: "mark_intercompany", intercompanyAction: "confirm", intercompanyLinkId: id });
    refetch();
  }
    ...
    <div className="flex flex-col min-h-full">
      ...
      <div className="flex flex-col gap-5 p-6 px-8 flex-1 overflow-auto">
        <div className="flex items-center justify-between">
          ...
          <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="h-8 px-3 text-[12px] border border-subtle rounded-md">
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className="bg-white rounded-lg border border-subtle p-4">
            ...
            <p className="text-[20px] font-semibold text-text-primary">{data?.pagination.total ?? 0}</p>
          </div>
        </div>

        {loading ? <LoadingSpinner /> : links.length === 0 ? (
          <EmptyState icon={ArrowLeftRight} title="Sin operaciones intercompañía" description="Las transferencias entre sociedades del grupo se detectan automáticamente." />
        ) : (
          <div className="bg-white rounded-lg border border-subtle overflow-hidden">
            ...
            {links.map((link) => (
              <div key={link.id} className="flex items-center h-11 px-5 border-b border-border-light text-[13px] hover:bg-page transition-colors">
                <span className="w-24 text-text-secondary">{new Date(link.date).toLocaleDateString("es-ES")}</span>
                <span className="w-36 text-text-primary font-medium truncate">{link.companyA?.shortName ?? link.companyA?.name ?? "—"}</span>
                <span className="w-8 text-center text-text-tertiary">→</span>
                <span className="w-36 text-text-primary font-medium truncate">{link.companyB?.shortName ?? link.companyB?.name ?? "—"}</span>
                <span className="w-28 text-right font-mono font-medium">{formatAmount(link.amount)}</span>
                <span className="flex-1 text-text-secondary truncate">{link.concept ?? "—"}</span>
                <span className="w-24"><Badge value={link.status} /></span>
                <span className="w-20 flex justify-end gap-1">
                  {link.status === "DETECTED" && (
                    <button onClick={() => handleConfirm(link.id)} className="p-1 rounded hover:bg-green-light text-green" title="Confirmar">
                      <Check size={14} />
                    </button>
                  )}
                </span>
