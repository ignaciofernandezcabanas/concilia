...
export const GET = withAuth(
  async (_req: NextRequest, ctx: AuthContext) => {
    ...
    const totals = runs.reduce(
      ...
      { txsProcessed: 0, txsAutoExecuted: 0, txsToBandeja: 0, llmCallsTotal: 0, llmCostEstimate: 0, errorsCount: 0 }
    );

    const automationRate = totals.txsProcessed > 0
      ? Math.round((totals.txsAutoExecuted / totals.txsProcessed) * 100)
      : 0;
...
const updateSchema = z.object({
  autoExecuteThreshold: z.number().min(0.80).max(0.99).optional(),
});
