...
export const GET = withAuth(async (req: NextRequest, ctx: AuthContext) => {
  ...
  try {
    ...
    const [data, total] = await Promise.all([
      db.inquiry.findMany({
        ...
        include: {
          ...
          bankTransaction: { select: { id: true, amount: true, concept: true, valueDate: true, counterpartName: true } },
    ...
    return NextResponse.json({ data, pagination: { total, page, pageSize, totalPages: Math.ceil(total / pageSize) } });
  } catch (err) {
