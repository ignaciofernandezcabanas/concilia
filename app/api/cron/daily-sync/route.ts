    ...
    for (const company of companies) {
      ...
      for (const [step, url] of [
        ...
        try {
          ...
          steps.push({ step, success: resp.ok, error: resp.ok ? undefined : (await resp.json().catch(() => ({}))).error });
        } catch (err) {
          steps.push({ step, success: false, error: err instanceof Error ? err.message : String(err) });
        }
      }
